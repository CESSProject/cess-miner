...
func Command_State_Runfunc(cmd *cobra.Command, args []string) {
	...
	var tableRows = []table.Row{
		...
		{"validated space", fmt.Sprintf("%v bytes", unitConversion(minerInfo.IdleSpace))},
		{"used space", fmt.Sprintf("%v bytes", unitConversion(minerInfo.ServiceSpace))},
		{"locked space", fmt.Sprintf("%v bytes", unitConversion(minerInfo.LockSpace))},
